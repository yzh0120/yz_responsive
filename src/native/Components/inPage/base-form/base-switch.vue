...
<template>
  <!-- <el-tooltip :content="tooltipTitle(item,data[item.field])" :popper-class="tooltipClass(item,data[item.field])" placement="right"> -->
    <el-switch @change="change" :active-value="item.av" :inactive-value="item.iav"  
	:inactive-text="textFalse()"  :active-text="textTrue()"  :inactive-color="colorFalse()" :active-color="colorTrue()" class="demo"
    v-model="data[item.field]"></el-switch>
  <!-- </el-tooltip> -->
...
export default {
  ...
  methods: {
    change(e) {
      // 默认 true 、false
      this.$emit("baseFormEvent",{event:'switch',value:e})
    },
